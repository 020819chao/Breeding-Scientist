...
class CandRow:
    ...
    wins: int
    losses: int
    mean_calibration_score: float | None
    top_calibration_score: float | None
...
def _load_candidates(con: sqlite3.Connection, bench_id: str) -> list[CandRow]:
    rows = con.execute(
        """SELECT id, label, provider, model, mode,
            ...
            WHERE bench_id=?
            ORDER BY (mean_calibration_score IS NULL), mean_calibration_score DESC, label""",
        (bench_id,),
    ).fetchall()
    out: list[CandRow] = []
    for r in rows:
...
def _winning_candidate_for_hyp(
    ...
    *summary* (not the title), truncated to 4000 chars, so we search by
    ...
    def _strip_mode_suffix(label: str) -> str:
        # Pipeline-vs-direct presets append "[pipe]" / "[direct]" to the candidate label
        # for readability in the runtime table; here we render it cleanly.
        for suffix in ("[pipe]", "[direct]"):
...
def _fmt_calibration_score(x: float | None) -> str:
...
def _bench_section(con: sqlite3.Connection, b: BenchRow) -> str:
    ...
    if cands:
        ...
        lines.append("")
        headers = ["label", "mode", "n_hyps", "W-L", "score",
        ...
        for c in cands:
            ...
            rows.append([
                ...
                _fmt_calibration_score(c.mean_calibration_score),
    ...
    if b.session_id:
        ...
        lines.append(f"- LLM transcripts (request + response per call): "
                     f"`data/artifacts/{ses}/transcripts/breeding_designer/`")
        if b.artifact_path:
        ...
        lines.append("       round(mean_calibration_score,0), gold_hits, gold_hit_names,")
...
def _headline_findings_section() -> str:
    """Static narrative summary of how to interpret breeding benches."""
    return "\n".join([
        "## Headline findings",
        "",
        "The `*-vs-direct` benches run each model twice on the same goal: ",
        "**direct** (a single design call) and **pipeline** (the full six-agent ",
        "breeding workflow with evidence tools, tool loops, review, validation ",
        "planning, and pairwise calibration), in one shared calibration pool.",
        "",
        "### 1. Treat small benches as smoke tests, not model verdicts",
        "",
        "With one or two hypotheses per candidate, a single strong or weak ",
        "route can dominate the pairwise pool. Use small benches to confirm ",
        "that the pipeline completes, produces structured breeding cards, and ",
        "surfaces evidence-linked clues. Use larger `--n` and `--matches` ",
        "before making a per-model claim.",
        "",
        "### 2. Gold-set hits measure clue coverage, not final correctness",
        "",
        "The bundled minor-grain demo gold sets check whether a candidate surfaced ",
        "key breeding clues such as markers, stress-response mechanisms, parent ",
        "materials, or validation gates. The current seed crop pack uses foxtail ",
        "millet examples; a hit means the hypothesis remembered a useful clue, "
        "not that the route is locally validated.",
        "",
        "### 3. Practical implications",
        "",
        "- **Don't read a single bench's calibration score as a model verdict.** The ",
        "  score is useful for debugging and coarse comparison, but serious ",
        "  comparisons need repeated seeds.",
        "- **Read gold-set misses as evidence-curation prompts.** If a model misses ",
        "  Seita.5G404900, SiDREB2-like, CAPS validation, or material checks, ",
        "  improve the local evidence graph, crop-pack RAG cards, or agent prompts.",
        "",
        "",
    ])
...
def _guess_preset(b: BenchRow) -> str:
    ...
    if "foxtail millet" in goal or "谷子" in goal:
        return "minor-grain demo"
...
def build_report(db_path: Path, out_path: Path) -> None:
    ...
    sections = [
        ...
        "   - the candidate result table (score, hits, $),",
        ...
        "can be rechecked against newer minor-grain clue sets after the local "
