...
def estimate(
    ...
    *,
    ...
    max_pairwise_checks_per_hypothesis: int | None = None,
) -> SessionEstimate:
    """Coarse pre-flight estimate. Heuristics live in this one place."""
    n_ideas = max_ideas or cfg.run.max_ideas
    pairwise_checks_per_hypothesis = (
        max_pairwise_checks_per_hypothesis
        or cfg.run.effective_max_pairwise_checks_per_hypothesis
    )
    n_pairwise_calibrations = n_ideas * pairwise_checks_per_hypothesis // 2
    n_route_revisions = int(n_ideas * 0.4)
    # n_reviews is implicit in the rows below (full + verification per hypothesis).
    n_meta_periodic = max(1, n_pairwise_calibrations // 50)

    rows = [
        CallEstimate(
            label="Goal Interpreter", n_calls=1, model=cfg.models.goal_interpreter,
            input_tokens_each=600, output_tokens_each=300, cache_ratio=0.0,
        ),
        CallEstimate(
            label="Breeding Designer", n_calls=n_ideas, model=cfg.models.breeding_designer,
            input_tokens_each=18_000, output_tokens_each=2_500, cache_ratio=0.55,
        ),
        CallEstimate(
            label="Risk Reviewer evidence review", n_calls=n_ideas, model=cfg.models.risk_reviewer_evidence,
            input_tokens_each=16_000, output_tokens_each=2_000, cache_ratio=0.55,
        ),
        CallEstimate(
            label="Risk Reviewer verification", n_calls=n_ideas,
            model=cfg.models.risk_reviewer_evidence,
            input_tokens_each=14_000, output_tokens_each=3_000, cache_ratio=0.55,
        ),
        CallEstimate(
            label="Pairwise calibration", n_calls=n_pairwise_calibrations,
            model=cfg.models.pairwise_calibration,
            input_tokens_each=8_000, output_tokens_each=800, cache_ratio=0.70,
        ),
        CallEstimate(
            label="Breeding Designer revision", n_calls=n_route_revisions, model=cfg.models.breeding_designer_revision,
            input_tokens_each=16_000, output_tokens_each=2_500, cache_ratio=0.50,
        ),
        CallEstimate(
            label="Iteration Orchestrator feedback", n_calls=n_meta_periodic,
            model=cfg.models.iteration_feedback,
            input_tokens_each=14_000, output_tokens_each=2_000, cache_ratio=0.50,
        ),
        CallEstimate(
            label="Final breeding synthesis", n_calls=1,
            model=cfg.models.final_synthesis,
    ...
    if total > cfg.run.budget_usd * 1.2:
        warn = (
            ...
            f"max_pairwise_checks_per_hypothesis, or raising --budget-usd."
        )
