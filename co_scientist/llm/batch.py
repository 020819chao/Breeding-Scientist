"""Anthropic Batch API support for lower-priority pairwise calibration checks.

The Batch API is ~50% cheaper than synchronous Messages calls but takes up to
24 h to complete. We use it for pairwise checks between lower-scoring routes
where the freshness of the verdict matters less; the strongest candidates stay
on the synchronous path so the final overview reflects fresh judgments.

This module owns:
- BatchedMatch dataclass + per-session in-memory queue of pending pairwise checks.
- submit_batch(): drains the queue into one batch request.
- poll_batch(): checks for completion + extracts verdicts.
- A small reconciler that turns completed batch results into pairwise
  calibration persistence rows.

The pairwise calibration helper decides whether to enqueue a check for batch
...
@dataclass
class BatchedMatch:
    """One pending pairwise calibration check awaiting batch submission."""

    custom_id: str               # e.g. "match::hyp_a::hyp_b::round_id"
    session_id: str
    hyp_a: str
    hyp_b: str
    prompt: str                  # iteration_orchestrator_pairwise_calibration.md rendered
    model: str
    max_tokens: int = 1024
    system: str = ""
...
class BatchPool:
    """In-memory queue of pending pairwise checks, drained by submit_batch().

    The Supervisor owns one pool. Checks are added by the pairwise calibration
    helper when it determines the pair can be batched, and the Supervisor periodically calls
