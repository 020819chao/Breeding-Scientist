...
For every Evidence{claim, url, excerpt} in a risk-review record:
...
class CitationVerifier:
    ...
    async def verify_review(
        self, session_id: str, review: Review, db
    ) -> dict[str, Any]:
        ...
        for ev in evidence:
            ...
            if not url or not excerpt:
                continue
            result = await self._fetcher.call({"url": url, "max_chars": 200_000}, ctx)
