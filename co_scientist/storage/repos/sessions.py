...
async def delete_cascade(conn: aiosqlite.Connection, session_id: str) -> bool:
    """Delete one terminal Session and all rows owned by it."""
    ...
    try:
        await conn.execute(
            "DELETE FROM pairwise_calibration_journal "
            "WHERE match_id IN ("
            "SELECT id FROM pairwise_calibration_matches WHERE session_id=?"
            ")",
            (session_id,),
        )
        await conn.execute("DELETE FROM pairwise_calibration_matches WHERE session_id=?", (session_id,))
        ...
        await conn.execute("DELETE FROM spans WHERE session_id=?", (session_id,))
        await conn.execute("DELETE FROM agent_output_reviews WHERE session_id=?", (session_id,))
        await conn.execute("DELETE FROM system_feedback WHERE session_id=?", (session_id,))
        await conn.execute("DELETE FROM reviews WHERE session_id=?", (session_id,))
        await conn.execute("DELETE FROM embeddings_meta WHERE session_id=?", (session_id,))
        await conn.execute("DELETE FROM tasks WHERE session_id=?", (session_id,))
        await conn.execute("DELETE FROM hypotheses WHERE session_id=?", (session_id,))
