...
async def exists_by_idempotency_key(
    ...
    """Return whether a task with this durable idempotency key already exists."""
...
async def cancel(
    ...
    """Cancel a task that cannot make progress in the current budget window."""

    await conn.execute(
        """UPDATE tasks
              SET status='cancelled', last_error=?, finished_at=?,
                  lease_owner=NULL, lease_expires_at=NULL
            WHERE id=?""",
        (error[:4000], _now(), task_id),
    )
    await conn.commit()
