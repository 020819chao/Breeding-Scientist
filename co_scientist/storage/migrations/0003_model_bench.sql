...
--
-- A bench run takes one research goal and runs the hypothesis-design pipeline
-- under several different (provider, model) configurations. All produced
-- hypotheses then enter a single shared pairwise-calibration pool judged by one
-- fixed judge model. The per-candidate aggregate calibration score + win-rate
-- matrix lets us compare models on a level field.
...
CREATE TABLE IF NOT EXISTS bench_candidates (
    ...
    mean_calibration_score REAL,
    top_calibration_score  REAL,
...
-- Bench pairwise checks are stored separately from the regular-session
-- pairwise calibration match table so they don't pollute per-session
-- prioritized route lists.
