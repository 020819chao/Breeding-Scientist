...
class NoEmbeddingsAvailable(RuntimeError):
    ...
    this and treat semantic deduplication as a soft no-op rather than failing
    the agent."""
...
class HashEmbedder:
    ...
    keeps semantic deduplication running rather than crashing the session.
    """
...
# pair-selection (make_embedder is called inside the calibration loop and was
...
def make_embedder(cfg: Config) -> Embedder:
    ...
    semantic quality in deduplication).
    """
