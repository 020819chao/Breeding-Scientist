...
from .prioritization.composite import calibration_score_to_ui_score as pairwise_calibration_score
from .storage import db as db_mod

VERSION = "0.1.0"

app = typer.Typer(
    help="AI Breeding Scientist 鈥?six-agent breeding hypothesis design, evidence curation, validation planning, risk review, and iterative closure.",
    no_args_is_help=True,
    add_completion=False,
)
tools_app = typer.Typer(help="Tool registry inspection and debug invocation.", no_args_is_help=True)
app.add_typer(tools_app, name="tools")
knowledge_app = typer.Typer(help="Validate and activate local breeding knowledge batches.", no_args_is_help=True)
...
@knowledge_app.command("validate-batch")
def validate_knowledge_batch_command(batch_dir: Path) -> None:
    """Validate one portable knowledge intake batch without changing live data."""
    from .knowledge.intake import validate_knowledge_batch

    report = validate_knowledge_batch(batch_dir)
    for check in report.checks:
        console.print(f"{'[green]OK[/green]' if check.ok else '[red]ERROR[/red]'} {check.name}: {check.message}")
    if not report.ok:
        raise typer.Exit(1)
    console.print(f"Knowledge batch validation passed: {report.batch_dir}")


@knowledge_app.command("import-batch")
def import_knowledge_batch_command(
    batch_dir: Path,
    target_root: Path | None = typer.Option(
        None,
        "--target-root",
        help="Active knowledge directory; defaults to data/knowledge/active.",
    ),
    catalog: Path | None = typer.Option(
        None,
        "--catalog",
        help="Catalog path; defaults to <target-root>/catalog.json.",
    ),
    ...
    """Validate, deduplicate, index, and safely activate a minor-grain batch."""
    from .knowledge.intake import DEFAULT_ACTIVE_ROOT, import_knowledge_batch

    try:
        result = import_knowledge_batch(
            batch_dir,
            active_root=target_root or DEFAULT_ACTIVE_ROOT,
            catalog_path=catalog,
            dry_run=dry_run,
        )
    except (OSError, ValueError) as exc:
        console.print(f"[red]Knowledge batch import failed:[/red] {exc}")
        raise typer.Exit(1) from exc

    console.print(f"Batch: {result.batch_id}")
    console.print(f"Active: {'yes' if result.activated else 'dry-run'}")
    console.print(f"Catalog: {result.catalog_path}")
    if result.activated:
        console.print(f"Import report: {result.active_root / 'last_import_report.json'}")
...
def list_sessions(ctx: typer.Context) -> None:
        ...
        try:
            async with conn.execute(
                """SELECT id, status, research_goal, created_at, updated_at,
                          ...
                          (SELECT MAX(calibration_score) FROM hypotheses WHERE session_id = s.id)
                              AS top_pairwise_calibration
                     FROM sessions s
    ...
    tbl.add_column("hypotheses", justify="right")
    tbl.add_column("pairwise calibration", justify="right")
    ...
    for r in rows:
        tbl.add_row(
            ...
            str(r["n_hyps"]),
            (
                f"{pairwise_calibration_score(r['top_pairwise_calibration']):.1f}"
                if r["top_pairwise_calibration"] is not None
                else "-"
            ),
...
def run(
    ...
    n_initial: int | None = typer.Option(
        None,
        "--n",
        help="Explicit number of initial hypothesis cards; otherwise use the goal interpretation.",
    ),
    max_hypotheses: int | None = typer.Option(
        None,
        "--max-hypotheses",
        help="Maximum total hypothesis cards allowed in this session.",
    ),
    ...
    """Start a fresh six-agent breeding session."""
    cfg, _ = ctx.obj
    if not has_llm_key(cfg):
        env_var = provider_key_env(cfg)
    ...
    console.print(
        ...
        f"max_pairwise_checks_per_hypothesis="
        f"{cfg.run.effective_max_pairwise_checks_per_hypothesis})[/dim]"
    )
    if est.warning:
...
def accept_session(
    ctx: typer.Context,
    session_id: str = typer.Argument(...),
    check_web: bool = typer.Option(
        False,
        "--check-web",
        help="Also probe the session, evidence-graph, and overview HTTP pages.",
    ),
) -> None:
    """Run the deterministic acceptance gate for a completed session."""
    cfg, _ = ctx.obj
    ...
    result = asyncio.run(run_session_acceptance(cfg, session_id, check_web=check_web))
    write_session_acceptance(cfg, result)
...
def eval_cmd(
    ...
    agent: str | None = typer.Argument(
        None,
        help="breeding_designer|iteration_orchestrator|overview, or a canonical eval key",
    ),
...
def bench_cmd(
    ...
    preset: str | None = typer.Option(
        ...
        help=(
            ...
            "'baseline' (model panel, pairwise calibration only), "
            "'minor-grain' (minor-grain drought demo + local gold-set recall), "
            "'minor-grain-vs-direct' and 'frontier-minor-grain-vs-direct'. "
    ...
    candidate: list[str] = typer.Option(
        ...
        help=(
            ...
            "(default) or `direct` (single design call, no tools). e.g. "
            "'gemini-flash=openrouter:google/gemini-3-flash-preview', "
            "'flash-direct=openrouter:google/gemini-3-flash-preview@direct'."
        ),
    ),
    n: int = typer.Option(2, "--n", help="Hypotheses per candidate."),
    matches: int = typer.Option(2, "--matches", help="Pairwise calibration checks per pair."),
    ...
    goldset_label: str | None = typer.Option(
        ...
        help=(
            ...
            "candidate bench. Built-in labels: 'minor-grain-drought-demo' "
            "and 'minor-grain-resource-demo'. Pass 'none' to "
    ...
    """Compare N models on the same goal via pairwise calibration.

    Quick start:
      co-scientist bench --preset minor-grain \\
        --n 2 --matches 2 \\
        --judge openrouter:google/gemini-3-flash-preview

    Custom candidates:
      co-scientist bench "Improve drought tolerance in a target minor grain" \\
    ...
    if preset:
        ...
        for c in candidates:
            ...
            console.print(f"[dim]  鈥?{c.label}{mode_suffix}: {c.provider}:{c.model}[/dim]")
    else:
        if not candidate:
            console.print(
    ...
    outcome = asyncio.run(
        run_bench(
            ...
            pairwise_checks_per_pair=matches,
    ...
    title = f"Bench {outcome.bench_id} 鈥?{outcome.pairwise_checks_played} pairwise checks"
    if has_gold:
        title += f" 鈥?gold-set {goldset.label} (recall / {len(goldset.entities)})"
    ...
    tbl.add_column("mean_pairwise_calibration", justify="right")
    if has_gold:
    ...
    for i, row in enumerate(outcome.candidates, 1):
        mean_calibration_score = row.get("mean_calibration_score")
        row_cells = [
            str(i), row["label"],
            ...
            (
                f"{pairwise_calibration_score(mean_calibration_score):.1f}"
                if mean_calibration_score is not None
                else "-"
            ),
        ]
        if has_gold:
