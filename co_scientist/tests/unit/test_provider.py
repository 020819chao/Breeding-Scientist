...
def test_adapt_response_repairs_unquoted_string_tool_arg() -> None:
    raw = _fake_openai_response(
        tool_calls=[
            {
                "id": "c",
                ...
                "arguments": (
                    '{"title": "T", "statement": "S", '
                    '"anticipated_outcomes": If the hypothesis is true, F2 lines improve lodging, '
                    '"breeding_context": {"crop": "foxtail millet"}}'
                ),
            }
        ],
        finish="tool_calls",
    ...
    tool_input = msg.content[0].input
    assert tool_input["statement"] == "S"
    assert tool_input["anticipated_outcomes"].startswith("If the hypothesis is true")
    assert tool_input["breeding_context"]["crop"] == "foxtail millet"
...
async def test_openai_call_persists_transcript_and_charges_budget(tmp_cfg, conn) -> None:
    ...
    try:
        ...
        assert snap["breeding_designer"]["used_usd"] > 0

        async with conn.execute(
            "SELECT agent, action, artifact_path FROM transcripts WHERE session_id=?",
            ("ses_p",),
        ) as cur:
            row = await cur.fetchone()
        assert row["agent"] == "breeding_designer"
        assert row["action"] == "test"
        normalized_artifact_path = row["artifact_path"].replace("\\", "/")
        assert "transcripts/breeding_designer" in normalized_artifact_path
    finally:
        pass
