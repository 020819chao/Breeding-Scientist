...
def test_six_agent_config_fields_are_primary() -> None:
    cfg = Config()
    assert cfg.models.goal_interpreter == "claude-sonnet-4-6"
    assert cfg.models.breeding_designer == "claude-opus-4-7"
    assert cfg.models.pairwise_calibration == "claude-sonnet-4-6"
    assert cfg.thinking.breeding_designer_literature == 4000
    assert cfg.thinking.route_combine == 6000
    assert cfg.budget_shares.breeding_designer == 0.35
    assert cfg.budget_shares.iteration_orchestrator == 0.17
    assert cfg.budget_shares.evidence_curator == 0.15
