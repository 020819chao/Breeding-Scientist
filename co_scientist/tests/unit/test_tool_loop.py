...
def _spec() -> AgentCallSpec:
    return AgentCallSpec(
        route=ModelRoute(agent="breeding_designer", mode="literature", model="x"),
        user_blocks=[CachedBlock("go")],
        tools=[
            {"name": "search", "description": "", "input_schema": {}},
            {"name": "record_hypothesis", "description": "", "input_schema": {}},
        ],
...
def _ctx() -> CallContext:
    return CallContext(session_id="s", task_id="t", agent="breeding_designer", action="a")
...
@pytest.mark.asyncio
async def test_loop_tracks_germplasm_tool_urls_and_sources() -> None:
    """germplasm_search returns local resource clues with a source URL that
    should be visible to citation filtering and finalization hints."""
    from co_scientist.tools.base import ToolResult
    ...
    client.call = AsyncMock(side_effect=[
        _fake_response(
            ...
            blocks=[{
                ...
                "input": {"query": "263A", "crop": "foxtail millet"},
            }],
        ),
        _fake_response(
            stop_reason="tool_use",
            ...
            "results": [
                {
                    ...
                    "name": "263A",
                    "url": "https://doi.org/10.1016/j.cj.2022.09.003",
                    "summary": "A semi-dwarf foxtail millet line.",
    ...
    assert result.seen_urls == {"https://doi.org/10.1016/j.cj.2022.09.003"}
    assert result.observed_sources == [
        {
            "url": "https://doi.org/10.1016/j.cj.2022.09.003",
            "title": "263A",
            "excerpt": "A semi-dwarf foxtail millet line.",
        }
    ]


@pytest.mark.asyncio
async def test_loop_tracks_local_rag_urls_and_sources() -> None:
    """Local RAG URLs are valid source URLs for citation filtering."""
    from co_scientist.tools.base import ToolResult
    ...
    registry.call = AsyncMock(return_value=ToolResult(
        ...
        content={
            ...
            "results": [
                {
                    "chunk_id": "sinfyc2_lodging_note.md#chunk-1",
                    "url": "local-rag://sinfyc2_lodging_note.md#L1-L4",
                    "title": "SiNF-YC2 and lodging architecture note",
                    "excerpt": "CAPS marker screening should be validated under dense planting.",
                }
            ],
        },
        duration_ms=1,
    ...
    assert result.seen_urls == {"local-rag://sinfyc2_lodging_note.md#L1-L4"}
    assert result.observed_sources == [
        {
            "url": "local-rag://sinfyc2_lodging_note.md#L1-L4",
            "title": "SiNF-YC2 and lodging architecture note",
