...
async def test_subscribe_receives_publish() -> None:
    ...
    await bus.publish("ses_a", "pairwise_calibration_complete", {"x": 1})
    await bus.publish("ses_a", "pairwise_calibration_complete", {"x": 2})
...
async def test_publishes_isolated_by_session() -> None:
    ...
    task = asyncio.create_task(reader())
    await asyncio.sleep(0.05)
    await bus.publish("ses_b", "pairwise_calibration_complete", {"x": "other"})
    await bus.publish("ses_a", "pairwise_calibration_complete", {"x": "mine"})
    await asyncio.wait_for(task, timeout=2.0)
    assert len(got_a) == 1
