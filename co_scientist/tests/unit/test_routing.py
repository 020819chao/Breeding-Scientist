...
def test_default_routes_use_opus_for_heavy_modes() -> None:
    cfg = Config()
    assert route(cfg, "breeding_designer", "literature").model == cfg.models.breeding_designer
    assert route(cfg, "breeding_designer", "combine").model == cfg.models.breeding_designer_revision
    assert route(cfg, "risk_reviewer", "evidence_review").model == cfg.models.risk_reviewer_evidence
    assert route(cfg, "iteration_orchestrator", "pairwise").model == cfg.models.pairwise_calibration
    assert route(cfg, "iteration_orchestrator", "final").model == cfg.models.final_synthesis
    assert route(cfg, "iteration_orchestrator", "final_repair").model == cfg.models.final_synthesis


def test_thinking_only_on_opus() -> None:
    cfg = Config()
    r = route(cfg, "risk_reviewer", "verification")
    if r.model.startswith("claude-opus"):
        assert r.thinking_tokens == cfg.thinking.verification_review
    else:
...
def test_degrade_walks_chain_once() -> None:
    cfg = Config()
    r1 = route(cfg, "breeding_designer", "literature", degraded=False)
    r2 = route(cfg, "breeding_designer", "literature", degraded=True)
    # Should be different unless never-degrade
    assert "breeding_designer.literature" not in NEVER_DEGRADE
    assert r1.model != r2.model


def test_never_degrade_modes_stay_put() -> None:
    cfg = Config()
    r1 = route(cfg, "risk_reviewer", "verification", degraded=False)
    r2 = route(cfg, "risk_reviewer", "verification", degraded=True)
    assert r1.model == r2.model
...
def test_thinking_budget_lookup() -> None:
    cfg = Config()
    assert (
        thinking_budget_for(cfg, "risk_reviewer.verification")
        == cfg.thinking.verification_review
    )
    assert (
        thinking_budget_for(cfg, "iteration_orchestrator.final_repair")
        == cfg.thinking.final_synthesis
    )
    assert thinking_budget_for(cfg, "breeding_designer.combine") == cfg.thinking.route_combine
