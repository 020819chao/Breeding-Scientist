...
async def _seed(conn, session_id: str = "ses_m") -> None:
    ...
    await hyp_repo.insert(conn, Hypothesis(
        ...
        state="calibration_pool", calibration_score=1234.0, pairwise_calibrations_played=3,
    ))
    await tx_repo.insert(conn, Transcript(
        id="trn_1", session_id=session_id, task_id=None,
        agent="breeding_designer", action="x", model="claude-opus-4-7",
        input_tokens=1000, output_tokens=200, cache_read=500, cache_write=0,
        cost_usd=0.12, started_at=now, finished_at=now,
        artifact_path=f"artifacts/{session_id}/transcripts/breeding_designer/trn_1.json",
    ))
...
async def test_session_metrics_aggregates(conn) -> None:
    ...
    assert m.n_in_calibration_pool == 1
...
@pytest.mark.asyncio
async def test_hypothesis_pairwise_calibration_aliases(conn) -> None:
    await _seed(conn, session_id="ses_alias")
    hyps = await hyp_repo.list_for_session(conn, "ses_alias")
    assert hyps[0].calibration_score == 1234.0
    assert hyps[0].pairwise_calibrations_played == 3
...
async def test_session_metrics_to_dict_roundtrips(conn) -> None:
    ...
    for key in (
        ...
        "n_in_calibration_pool", "cost_usd", "cache_hit_ratio",
        "n_pairwise_calibrations", "n_invalid_pairwise_calibrations",
    ):
        assert key in d
