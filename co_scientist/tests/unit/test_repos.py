...
async def test_hypothesis_insert_or_ignore_dedupes_on_deterministic_id(conn) -> None:
    ...
    hid = ids.hypothesis_id(s.id, "breeding_designer/literature", statement)
    h = Hypothesis(
    ...
    # Same statement 鈫?same id 鈫?INSERT OR IGNORE returns False
...
@pytest.mark.asyncio
async def test_queue_pairwise_calibration_only_runs_once(conn) -> None:
    s = await _make_session(conn)
    hid = ids.hypothesis_id(s.id, "breeding_designer/literature", "h")
    ...
    assert (
        await hyp_repo.queue_pairwise_calibration(
            conn, hid, initial_calibration_score=1200
        )
        is True
    )
    # second call must be a no-op because calibration score is already set.
    assert (
        await hyp_repo.queue_pairwise_calibration(
            conn, hid, initial_calibration_score=9999
        )
        is False
    )
    h = await hyp_repo.fetch(conn, hid)
    assert h is not None and h.calibration_score == 1200
    assert h.state == "calibration_pool"
    assert (
        await hyp_repo.queue_pairwise_calibration(
            conn, hid, initial_calibration_score=9999
        )
...
@pytest.mark.asyncio
async def test_calibration_pool_listing_filters_to_calibration_state(conn) -> None:
    s = await _make_session(conn)
    hid = ids.hypothesis_id(s.id, "breeding_designer/literature", "migrated-state")
    await hyp_repo.insert(conn, Hypothesis(
        id=hid, session_id=s.id, created_at=_now(),
        created_by="breeding_designer", strategy="literature",
        title="t", summary="s", full_text="f",
    ...
    listed = await hyp_repo.list_for_session(conn, s.id, state="calibration_pool")
...
async def test_set_state_if_only_applies_when_expected(conn) -> None:
    ...
    s = await _make_session(conn)
    hid = ids.hypothesis_id(s.id, "breeding_designer/literature", "h-state")
    await hyp_repo.insert(conn, Hypothesis(
        id=hid, session_id=s.id, created_at=_now(),
        created_by="breeding_designer", strategy="literature",
        title="t", summary="s", full_text="f",
    ...
    # draft 鈫?reviewed: allowed
    applied = await hyp_repo.set_state_if(
        conn, hid, new_state="reviewed", expected_states=("draft",)
    )
    assert applied is True

    # Promote past evidence review into pairwise calibration.
    await hyp_repo.queue_pairwise_calibration(conn, hid, initial_calibration_score=1200)

    # Evidence review re-fires: must NOT drag calibration_pool 鈫?reviewed.
    applied2 = await hyp_repo.set_state_if(
        conn, hid, new_state="reviewed", expected_states=("draft",)
    )
    assert applied2 is False
    h = await hyp_repo.fetch(conn, hid)
    assert h is not None and h.state == "calibration_pool"
...
async def test_task_queue_claim_and_idempotency(conn) -> None:
    ...
    t = Task(
        ...
        agent="risk_reviewer", action="AssessHypothesisEvidence", target_id="hyp_z",
...
@pytest.mark.asyncio
async def test_task_cancel_records_deferred_reason(conn) -> None:
    s = await _make_session(conn)
    t = Task(
        id=ids.task_id(), session_id=s.id, created_at=_now(),
        agent="risk_reviewer", action="AssessHypothesisEvidence", target_id="hyp_z",
    ...
    await task_repo.cancel(conn, t.id, "pending_review: budget exhausted")
    async with conn.execute("SELECT status, last_error FROM tasks WHERE id=?", (t.id,)) as cur:
        row = await cur.fetchone()
    assert row["status"] == "cancelled"
    assert row["last_error"] == "pending_review: budget exhausted"
