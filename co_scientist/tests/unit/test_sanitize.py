...
from co_scientist.web.app import _link_overview_references
from co_scientist.web.sanitize import render_markdown, sanitize_html
...
def test_link_overview_references_makes_sources_and_hypotheses_clickable() -> None:
    md = (
        "证据 [Source: https://doi.org/10.123/example]. "
        "假设 [H-hyp_abc123] and [H- hyp_spaced123] and `hyp_def456`."
    )

    linked = _link_overview_references(md, "ses_test")
    html = render_markdown(linked)

    assert 'href="https://doi.org/10.123/example"' in html
    assert 'href="/sessions/ses_test/hypotheses/hyp_abc123"' in html
    assert 'href="/sessions/ses_test/hypotheses/hyp_spaced123"' in html
    assert 'href="/sessions/ses_test/hypotheses/hyp_def456"' in html
    assert "Source: [https://" not in html
