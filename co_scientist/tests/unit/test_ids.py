...
def test_hypothesis_id_is_deterministic_per_normalized_statement() -> None:
    a = ids.hypothesis_id("ses_1", "breeding_designer/literature", "  Hypothesis X involves Y. ")
    b = ids.hypothesis_id("ses_1", "breeding_designer/literature", "hypothesis x involves y.")
    assert a == b


def test_hypothesis_id_changes_when_session_or_origin_changes() -> None:
    a = ids.hypothesis_id("ses_1", "breeding_designer/literature", "Hypothesis X.")
    b = ids.hypothesis_id("ses_2", "breeding_designer/literature", "Hypothesis X.")
    c = ids.hypothesis_id("ses_1", "breeding_designer/combine", "Hypothesis X.")
    assert a != b
    assert a != c
