...
async def test_write_json_accepts_slash_in_kind(tmp_path: Path) -> None:
    """Legitimate callers use kinds like 'transcripts/breeding_designer'."""
    cfg = _cfg(tmp_path)
    rel = await artifacts.write_json(
        cfg, "sess1", "transcripts/breeding_designer", "abc", {"k": 1}
    )
    assert "transcripts/breeding_designer" in rel.replace("\\", "/")
