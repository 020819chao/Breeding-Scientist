...
- must_cite_at_least: int (risk_reviewer_evidence only)

Run:
    co-scientist eval <agent>             — score all fixtures with the judge
    co-scientist eval <agent> --offline   — only check structural assertions
"""
...
from .rubrics import (
    BREEDING_DESIGNER_RUBRIC,
    EVIDENCE_REVIEW_RUBRIC,
    OVERVIEW_RUBRIC,
    PAIRWISE_CALIBRATION_RUBRIC,
    RubricCriterion,
    judge,
)

log = get_logger("evals")
FIXTURES_DIR = PROJECT_ROOT / "co_scientist" / "evals" / "fixtures"

RUBRICS: dict[str, list[RubricCriterion]] = {
    "breeding_designer": BREEDING_DESIGNER_RUBRIC,
    "risk_reviewer_evidence": EVIDENCE_REVIEW_RUBRIC,
    "pairwise_calibration": PAIRWISE_CALIBRATION_RUBRIC,
...
def _check_structure(agent: str, candidate: str, expected: dict[str, Any]) -> list[str]:
    ...
    if agent == "pairwise_calibration" and "better idea:" not in candidate.lower():
        errors.append("pairwise calibration output must end with 'better idea: 1|2'")
    if agent == "risk_reviewer_evidence":
...
def _main() -> None:  # pragma: no cover
    ...
    parser = argparse.ArgumentParser(prog="co-scientist-evals")
    parser.add_argument(
        "agent",
        nargs="?",
        help="breeding_designer|risk_reviewer_evidence|pairwise_calibration|overview, or omit for all",
    )
